...
export const eventAPI = {
  ...
  // Get current active event (for home page)
  getCurrentActiveEvent: async () => {
    try {
      console.log('Fetching current active event...');
      const response = await api.get('/events/active');
      console.log('Active event fetched:', response.data);
      return response;
    } catch (error) {
  ...
  getActiveEvents: async () => {
    try {
      ...
      console.error('Get active events failed:', error);
      throw error;
    }
  },
  ...
  // Get all events for admin/manager (with pagination and filters)
  getAllEventsForAdmin: async (params = {}) => {
    try {
      ...
      console.error('Get all events for admin failed:', error);
      throw error;
    }
  },
  ...
  toggleEventActive: async (eventId) => {
    try {
      console.log('Toggling event active status:', eventId);
      const response = await api.patch(`/events/${eventId}/toggle-active`);
      console.log('Event status toggled:', response.data);
      return response;
    } catch (error) {
      console.error('Toggle event active failed:', error);
      throw error;
    }
  },
  
  // Get event statistics (admin/manager only)
...
export const apiUtils = {
  ...
  testEventEndpoints: async () => {
    try {
      // Test public endpoint
      const activeEvents = await eventAPI.getActiveEvents();
      
      // Test protected endpoint (will fail if not authenticated)
      let adminEvents = null;
      try {
        adminEvents = await eventAPI.getAllEventsForAdmin({ page: 1, limit: 5 });
      } catch (authError) {
        console.log('Protected endpoint test - authentication required (expected)');
      }
      
      return {
        success: true,
        publicEndpoint: !!activeEvents,
        protectedEndpoint: !!adminEvents,
        data: {
          activeEventsCount: activeEvents?.data?.count || 0,
          adminEventsCount: adminEvents?.data?.data?.length || 'N/A (auth required)'
        }
      };
    } catch (error) {
      return { success: false, error: error.message };
