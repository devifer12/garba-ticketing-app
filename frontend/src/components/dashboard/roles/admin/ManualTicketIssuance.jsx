...
const ManualTicketIssuance = ({ userRole }) => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [usersResponse, eventResponse] = await Promise.all([
        adminAPI
          .getAllUsers({ limit: 100, role: "guest" })
          .catch(() => ({ data: { users: [] } })),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const ticketData = {
        ...
        ticketNames: showNameFields
          ? ticketNames.filter((name) => name.trim())
          : [],
      };
      ...
      if (response.data.success) {
        ...
        toast.success(
          `🎉 ${formData.quantity} ticket(s) issued successfully for ${response.data.user.name}!`
        );
      }
    } catch (error) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      toast.info("🔄 Generating individual PDFs for each ticket...", {
        autoClose: 3000,
      });

      // Use the new API service function that returns a response with blob data
      const response = await adminAPI.generateSeparateTicketPDFs(ticketIds);

      // Create download link from the response data (which is already a blob)
      ...
      const sanitizedUserName = issuedTickets.user.name
        .replace(/[^a-zA-Z0-9\s]/g, "")
        .replace(/\s+/g, "-")
        .toLowerCase();

      a.download = `garba-tickets-${sanitizedUserName}-${timestamp}.zip`;
      ...
      toast.success(
        `📦 Downloaded ${issuedTickets.tickets.length} individual PDF tickets in ZIP file!`,
        {
          autoClose: 5000,
        }
      );
    } catch (error) {
      console.error("Failed to download separate PDFs:", error);
      const errorMessage = apiUtils.formatErrorMessage(error);
      toast.error(`Failed to download PDF tickets: ${errorMessage}`);
    } finally {
        ...
        <div className="bg-slate-800/30 backdrop-blur-xl rounded-3xl p-8 border border-slate-700/30">
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">
              Loading Manual Ticket Issuance
            </h2>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-slate-800/30 backdrop-blur-xl rounded-3xl p-8 border border-slate-700/30">
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">
              Please create an event first before issuing tickets.
            </p>
          </div>
        </div>
      </div>
        ...
        {!showConfirm ? (
          ...
          <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleNewUserDataChange("name", e.target.value)
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleNewUserDataChange("email", e.target.value)
                        }
                        className="w-full px-4 py-3 bg-slate-700/50 border border-slate-600/30 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500/30"
                      />
                    </div>
                  ...
                  {newUserData.name && newUserData.email && (
                    <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-white font-medium">
                            New User: {newUserData.name}
                          </p>
                          <p className="text-blue-300 text-sm">
                            {newUserData.email}
                          </p>
                          <p className="text-blue-200 text-xs">
                            Will be created as Guest user
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-4">
                    ...
                    <p className="text-blue-200 text-sm mb-4">
                      Enter individual names for each ticket. Leave blank to use
                      "{selectedUser?.name || newUserData.name}" for that
                      ticket.
                    </p>
            ...
            <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-6">
              <h4 className="text-blue-300 font-medium mb-4 text-lg">
                💰 Pricing Calculation
              </h4>
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("paymentDone", e.target.checked)
                    }
          ...
          <div className="space-y-6">
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">
                Confirm Manual Ticket Issuance
              </h2>
              <p className="text-slate-400">
                Please review the details before issuing tickets
              </p>
            </div>
            ...
            <div className="bg-slate-700/50 rounded-xl p-6 space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={`font-medium ${
                    formData.paymentDone ? "text-green-400" : "text-yellow-400"
                  }`}>
              ...
              {showNameFields && ticketNames.some((name) => name.trim()) && (
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {Array.from({ length: formData.quantity }, (_, index) => {
                      const ticketName =
                        ticketNames[index]?.trim() ||
                        selectedUser?.name ||
                        newUserData.name;
                      return (
                        <div
                          key={index}
                          className="text-white bg-slate-600/30 rounded p-2 text-sm">
                          Ticket #{index + 1}:{" "}
              ...
              {formData.notes && (
                <div>
                  ...
                  <p className="text-white bg-slate-600/30 rounded p-3 mt-2">
                    {formData.notes}
                  </p>
                </div>
              )}
          ...
          <motion.div
            ...
            <div className="p-6">
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-slate-400">
                  {formData.quantity} ticket(s) have been issued to{" "}
              ...
              <div className="bg-blue-900/20 border border-blue-700/30 rounded-lg p-4 mb-6">
                ...
                <p className="text-blue-200 text-sm">
                  Ticket confirmation email has been sent to{" "}
                  {issuedTickets.user.email}
                </p>
              </div>
              ...
              <div className="bg-purple-900/20 border border-purple-700/30 rounded-lg p-4 mb-6">
                ...
                <p className="text-purple-200 text-sm mb-4">
                  Would you like to download a PDF copy of the tickets for
                  printing or record keeping?
                </p>
